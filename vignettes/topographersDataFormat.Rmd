...
The next table shows the data file format required by **rPACI**'s `readCornealTopography`, each row represents a line, and the file should be a plain text document (with any extension, not necessarily *'.txt'*). In the file, $\rho$ values will be positive real numbers, and $z$ values will be real numbers. 
...
In addition, the R package **rPACI** includes also the functions `readDataset` which is able to read a file structured in the same way as the datasets used in the package. A file of this type is built using `writeDataset` over an **rPACI** dataset, which may have been obtained reading data from a file in other format (e.g. using `readCornealTopography`), or by simulation (with `simulateData`).  

Assuming there are $N$ full rings in the dataset and $N_A$ points in each ring, the file structure would be that of the table below. The functions `readDataset` and `writeDataset` are more flexible, though, and they can manage datasets with incomplete rings or a different number of points in each ring.
