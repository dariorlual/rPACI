...
The number of rings in the actual Placido disk $N_R$, which is projected into the eye, and the number of angles $N_A$ (or number of points per rings) in the digitization process may vary from a device to another. The coordinates of these points are the primary information given by the topographers. Afterward, they usually post-process the data to obtain altimetric (elevation) or curvature data and provide color maps and other easy-to-interpret indices to the practitioner. In some topographers, the raw measurements can be exported as a structured text file.
...
## Data file format read by rPACI


The R package **rPACI** includes a function able to read a raw Placido-disk corneal topography from a file that has been exported by certain corneal topographers. It has been especially designed for the file format exported from topographers manufactured by [CSO](https://www.csoitalia.it) (Firenze, Italy): EyeTop2005, Sirius, Antares, or Osiris-T, among others. 
 
 
The raw measurement file, exported by a corneal topographer, has to be in the following structure to be read by **rPACI**:

* A header with meta-data: patient data, corneal measurements, date of exploration, etc. Its size
depends on the device. For CSO topographers, the header typically has 24 lines. The function used in **rPACI** to read the files, `readCornealTopography`, is able to detect the header lines (those that are not convertable to real numbers) and to skip them.
...
* A list of size $N_R \times N_A$ with the $z$ coordinate, which can be altitude (elevation) or curvature, depending on the topographer and the exportation settings. These data are post-processed by the device,
not directly measured, and the $z$ values are not used in **rPACI**.

More details about using `readCornealTopography` and the rest of the **rPACI** package can be found in [Workflow of the rPACI package](packageUsage.html).

The next table shows the data file format required by **rPACI**, each row represents a line, and the file should be a plain text document (with any extension, not necessarily *'.txt'*). In the file, $\rho$ values will be positive real numbers, and $z$ values will be real numbers. 
